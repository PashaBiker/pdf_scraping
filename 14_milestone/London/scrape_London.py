    ...
    for key, url in file_urls.items():
        ...
        try:
            with requests.Session() as session:
                ...
                trs = soup.find_all('tr', class_='post-type-dlp_document')
                # Найти все tr с классами 'post-row' и 'post-type-dlp_document'
                for tr in trs:
                    print(tr)
                    # Извлечь текст из ячейки td с классом 'col-title'
                    td_element = tr.find('td')
                    desired_text = td_element.get_text(strip=True)

                    name = desired_text.strip()
                    # Убрать окончание после последнего символа "_"
                    if '_' in name:
                        name = name.rsplit("_", 1)[0]
                    
                    # Извлечь ссылку
                    link = tr.find('a', class_='dlp-download-link')['href']

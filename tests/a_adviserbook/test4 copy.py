...
def process_postcode(postcode, results_queue, session, headers, cookies, csrf_token):
    script_tagresponse = session.get(f'https://adviserbook.co.uk/financial-advisers/{postcode}', headers=headers, cookies=cookies)
...
column_data = read_json_file('updated_companies.json')

# Multithreading setup
num_threads = 10  # Adjust this number based on your requirements
results_queue = Queue()
threads = []

# Start threads
for cell_value in column_data:
    thread = threading.Thread(target=process_postcode, args=(cell_value['office_url'], results_queue, session, headers, cookies, csrf_token))
...
with open('tests/a_adviserbook/managers_output.json', 'w', encoding='utf-8') as file:

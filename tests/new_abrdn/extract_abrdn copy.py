...
def get_data(file):
    ...
    # mps_pattern = re.compile(r"^(Index\s+)?MPS\s+\d")
    # mps_pattern = re.compile(r"MPS\s+\d+")
    ...
    for line in text:
        ...
        # search_result = mps_pattern.search(line.strip())
        # if search_result and not found_mps:
        #     # Splitting the line at the pattern and then by spaces
        #     perf_values = line.split(search_result.group())[1].strip().split()
        ...
        for key in data:
            ...
            if "date" not in key.strip().lower():
                if data[key] is not None:
                    ...
                    # Установите значение ячейки в 0 или другое подходящее значение по умолчанию,
                    # если data[key] равно None
...
if __name__ == "__main__":
    ...
    # data, assets = get_data(pdfs[-2])
    # write_to_sheet(data, assets, pdfs[0].split('\\')[-1].split('.')[0], excel_file)

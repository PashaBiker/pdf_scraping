...
def get_data_worker(q, result_queue):
    while not q.empty():
        pdf = q.get()
        try:
            print(pdf)
            data = get_data(pdf)
            result_queue.put((data, pdf.split('\\')[-1].split('.')[0]))
        except Exception as e:
...
def write_to_sheet_worker(q):
    while not q.empty():
        data, filename = q.get()
        try:
            write_to_sheet(*data, excel_file, filename, data[-1])
        except Exception as e:
            print(f"An error occurred in file {filename}: {str(e)}")
        q.task_done()
...
if __name__ == '__main__':
    ...
    # Create threads for get_data_worker
    num_data_threads = 4
    for _ in range(num_data_threads):
        threading.Thread(target=get_data_worker, args=(data_queue, write_queue)).start()

    # Populate data_queue with PDF files
    for pdf in pdfs:
        data_queue.put(pdf)

    data_queue.join()

    # Create threads for write_to_sheet_worker
    num_write_threads = 2
    for _ in range(num_write_threads):
        threading.Thread(target=write_to_sheet_worker, args=(write_queue,)).start()

    # Wait for write_queue to be processed

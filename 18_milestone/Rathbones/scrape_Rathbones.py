    ...
    for key, url in file_urls.items():
        ...
        try:
            with requests.Session() as session:
                ...
                # print(soup)
                div_tag = soup.find('div', class_='coh-wysiwyg')
                # print(div_tag)
                # If the div exists and has a table inside
                if div_tag and div_tag.table:
                    # Find all 'a' tags within the table
                    a_tags = div_tag.table.find_all('a')

                    # Loop through the found 'a' tags and store their href and text
                    for a_tag in a_tags:
                        url = a_tag['href']

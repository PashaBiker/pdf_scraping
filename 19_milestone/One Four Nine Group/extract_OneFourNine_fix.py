    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for i,line in enumerate(first_page_text):
            ...
            if 'Underlying Fund Fees' in line:
                # Scan the next few lines for the fee
                for j in range(i, min(i + 3, len(first_page_text))):
                    if "%" in first_page_text[j]:
                        UF_fee_line = re.findall(r'\d\.\d{2}%', first_page_text[j])
                        if UF_fee_line:
                            UF_fee = UF_fee_line[0].replace('%', '')
                            print(UF_fee, 'uf fee')
            ...
            if '3 Years' in line:
                ...
                # Modified regular expression to include '-'
                numbers = re.findall(r'-?\d+\.\d+|—|N/A|-', numbers_line)
                print("Extracted numbers:", numbers)  # Debugging information

                # Convert numbers to float, None if 'N/A', or dash
                numbers = [float(num) if num.replace('.', '', 1).isdigit() else None for num in numbers]

...
def ocr_from_pdf(pdf_path):
    ...
    for i in range(len(images)):
        # Применяем Tesseract к каждому изображению
        # text = pytesseract.image_to_string(images[i], config=custom_config, lang='eng')
        # all_text += text + '\n'

        new_width = int(images[i].width * (1/3))
        new_height = int(images[i].height * (2/3))
        cropped_image = images[i].crop((0, 0, new_width, new_height))
        ...
        picture_text = re.sub(r'I Fixed a 38% Threadne', 'I Fixed interest 38% Threadne', picture_text)

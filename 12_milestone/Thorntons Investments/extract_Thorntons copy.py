...
import pdfplumber
import io
excel_file = 'Thorntons Investments.xlsm'
pdf_folder = 'Thorntons PDFs'
...
def get_data(file):
    ...
    asset_labels = ['International Equities',
                    'Other',
                    'UK Equities',
                    'Bonds',
                    'Property',
    ...
    with pdfplumber.open(file) as pdf:
        ...
        left_edge = page.width * 3/8
        right_edge = page.width * 5/8
        
        middle_section = page.crop((left_edge, 0, right_edge, page.height))
        text = middle_section.extract_text()  # Извлекаем текст с этой части страницы
        lines = text.split('\n')
        print(lines)

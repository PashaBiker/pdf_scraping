...
def get_data(file):
    ...
    for i, line in enumerate(text):
        if 'WEIGHTS BY ASSET' in line:
            ...
            continue  # Skip the current line, so it doesn't get added to current_group

        # If we find the line 'TOP 10 PERFORMANCE CONTRIBUTORS OVER 1 YEAR', stop adding
        elif 'TOP 10 PERFORMANCE CONTRIBUTORS OVER 1 YEAR' in line:
            found_portfolio_holdings = False
            
            # If the current_group has data, append it to the groups list
            if current_group:
                groups.append(current_group)
                current_group = []

        # If we're inside the block we're interested in, add lines
        elif found_portfolio_holdings:
            line_to_append = line.strip()

            if "Credit" in line_to_append:
                for idx, val in enumerate(current_group):
                    if "Fixed Income Multi Asset" in val:
                        percentage_part = val.split('Fixed Income Multi Asset ')[1]
                        current_group[idx] = 'Fixed Income Multi Asset Credit ' + percentage_part
                        break
            else:
                current_group.append(line_to_append)

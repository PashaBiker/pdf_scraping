...
def get_data(file):
        ...
        with pdfplumber.open(file) as pdf:
            ...
            for page in pdf.pages:
                ...
                for i in range(len(text) - 1):
                    line = text[i]
                    if 'TAM AMC' in line or 'Management Charge' in line or 'TAM Annual' in line:
                        AMC_number = re.findall(r'\d+\.\d+', line)
                        
                        if not AMC_number:  # Если число не найдено в текущей строке
                            AMC_number = re.findall(r'\d+\.\d+', text[i + 1])
                        
                        if AMC_number:  # Для избежания ошибок, проверим, что список не пустой
                            AMC = AMC_number[0]

                    if 'Underlying OCF' in line:
                        OCF_Number = re.findall(r'\d+\.?\d*', line)
                        OCF = OCF_Number[0]
                        date_match = re.search(r"(\d+\s[A-Z]+\s\d+)", text[2])
                        if date_match:
    ...
    asset_labels = ['Global Fixed Interest',
    ...
    with pdfplumber.open(file) as pdf:
        ...
        text = right_half.extract_text()  # Извлекаем текст со страницы
        # print(page_text)
